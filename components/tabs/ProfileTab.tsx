...
// استدعينا ايقونات جديدة للنافذة
import {
    User, Mail, Globe, LogOut, Sparkles, Coins, Settings, ChevronRight, X, Search, Check, Minus, Clock, Phone, Calendar, Flag
} from "lucide-react";
import {
...
export default function ProfileTab({ isActive }: ProfileTabProps) {
    ...
    const changeLanguage = (lang: string) => {
        // تحديث كوكي اللغة وإعادة التحميل لتطبيق التغيير
                ...
                {showLanguageModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        ...
                        >
                            ...
                            <h2 className="text-xl font-bold mb-6 text-gray-900 dark:text-white flex items-center gap-2">
                                <Globe className="text-[#D4AF37]" /> Language / اللغة
                            </h2>
                            <div className="space-y-3">
                                <button onClick={() => changeLanguage('ar')} className="w-full p-4 rounded-xl border border-gray-200 hover:border-[#D4AF37] hover:bg-[#D4AF37]/5 transition-all text-center font-bold text-lg">
                                    العربية
                                </button>
                                <button onClick={() => changeLanguage('en')} className="w-full p-4 rounded-xl border border-gray-200 hover:border-[#D4AF37] hover:bg-[#D4AF37]/5 transition-all text-center font-bold text-lg">
                                    English
                                </button>
                            </div>
                        </motion.div>
            ...
            <div className="max-w-2xl mx-auto space-y-6 pt-6">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="relative backdrop-blur-xl bg-white/40 dark:bg-white/5 rounded-3xl p-6 border border-white/20 dark:border-white/10 shadow-xl">
                    ...
                    <div className="space-y-3">
                        ...
                        <button onClick={() => setShowProfileModal(true)} className="w-full p-4 rounded-xl bg-white/50 hover:bg-white/70 transition-all text-left flex items-center justify-between" dir="rtl">
                            <div className="flex items-center gap-3">
                                <User className="w-5 h-5 text-[#D4AF37]" />
                                <span className="text-sm font-medium text-gray-900">إكمال بيانات الحساب</span>
                            </div>
                            <ChevronRight className="w-5 h-5 text-gray-400 rotate-180" />
                        </button>

                        {/* زر فتح مودال اللغة */}
                        <button onClick={() => setShowLanguageModal(true)} className="w-full p-4 rounded-xl bg-white/50 hover:bg-white/70 transition-all text-left flex items-center justify-between" dir="rtl">
                            <div className="flex items-center gap-3">
                                <Globe className="w-5 h-5 text-blue-500" />
                                <span className="text-sm font-medium text-gray-900">لغة التطبيق (Language)</span>
                            </div>
                            <ChevronRight className="w-5 h-5 text-gray-400 rotate-180" />
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} onClick={handleLogout} className="w-full py-4 rounded-2xl bg-red-50 text-red-600 font-bold hover:bg-red-100 transition-all flex items-center justify-center gap-2">
