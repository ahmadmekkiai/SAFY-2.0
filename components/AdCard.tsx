...
// هنا تم إضافة isSaved و onToggleSave لحل مشكلة الفيرسل نهائياً
...
export default function AdCard({ campaign, index, onRewardEarned, isSaved, onToggleSave }: AdCardProps) {
  ...
  // استخراج الداتا مع وجود قيم احتياطية
  ...
  // مراقبة ظهور الكارت في منتصف الشاشة لبدء التايمر
  useEffect(() => {
  ...
  return (
    <div ref={cardRef} className={`relative bg-white dark:bg-slate-800 rounded-3xl shadow-sm border overflow-hidden transition-all duration-500 ${isActive ? 'border-[#D4AF37] scale-[1.02] shadow-xl z-10' : 'border-gray-100 dark:border-slate-700 scale-100 opacity-95'}`}>
      ...
      >
        {isSaved ? (
          <FaHeart className="text-red-500 text-xl animate-in zoom-in" />
        ) : (
      ...
      <div className="relative h-52 w-full bg-gray-200">
        ...
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-black/20 z-10">
          <div 
            className="h-full bg-gradient-to-r from-[#D4AF37] to-[#F3E5AB] transition-all duration-1000 ease-linear shadow-[0_0_8px_#D4AF37]"
        ...
        <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm text-white px-3 py-1.5 rounded-2xl text-xs font-black flex items-center gap-1.5 z-10">
          {isCompleted ? (
            <span className="text-[#D4AF37] flex items-center gap-1">
              <FaCheckCircle /> {isAr ? "مكتمل" : "Completed"}
            </span>
          ) : (
            <span className={isActive ? "text-[#D4AF37] animate-pulse" : ""}>
              {timeLeft} {isAr ? "ثانية" : "sec"}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1 font-bold">
              <MdLocationOn className="text-[#D4AF37]" size={18} />
              {neighborhood}
            </p>
          </div>
          <span className="bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20 text-xs px-3 py-1.5 rounded-xl font-black shadow-sm">
            {distance} {isAr ? "كم" : "km"}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-gray-100 dark:border-slate-700">
          <div className="flex items-center gap-6">
            <button className="flex items-center gap-2 text-sm font-bold text-gray-600 dark:text-gray-300 hover:text-[#D4AF37] transition-all active:scale-95">
              <MdRestaurantMenu size={20} className="text-[#D4AF37]" />
              {isAr ? "المنيو" : "Menu"}
            </button>
            ...
            <div className="flex items-center gap-4 border-r border-gray-100 dark:border-slate-700 pr-4">
                ...
                className={`transition-all active:scale-125 ${likeStatus === "like" ? "text-green-500 scale-110" : "text-gray-400 hover:text-green-500"}`}
                >
                <FaThumbsUp size={20} />
                </button>
                ...
                className={`transition-all active:scale-125 ${likeStatus === "dislike" ? "text-red-500 scale-110" : "text-gray-400 hover:text-red-500"}`}
                >
      ...
      {showOverlay && (
        <div className="absolute inset-0 z-30 bg-[#D4AF37]/95 flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500 backdrop-blur-md">
          <div className="bg-white p-6 rounded-full shadow-2xl mb-4 animate-bounce">
            <FaCheckCircle className="text-[#D4AF37] text-6xl" />
          </div>
          <h2 className="text-white font-black text-3xl mb-1">
            {isAr ? "كفو عليك!" : "Awesome!"}
          </h2>
          <p className="text-white/90 text-lg font-bold">
            {isAr ? `كسبت ${rewardPoints} نقطة` : `You earned ${rewardPoints} Pts`}
          </p>
        </div>
      )}
    </div>
